defmodule TeamGeneratorTest do
  ...
  test "fixed number of teams created" do
    ...
    assert TeamGenerator.empty_teams(%{:team_type => :fixed, :options => 4}) == expected_result
  end

  test "two members generates two Teams" do
    [a1, a2] = generate_members(2)
    expected_result = get_teams(4)
    expected_result = [
              %{:team => 1, :names => [a1]},
              %{:team => 2, :names => [a2]},
              %{:team => 3, :names => []},
    ...
    assert TeamGenerator.allocate_members(get_teams(4), [a1, a2]) == expected_result
  end
  ...
  test "eight members generates 4 Teams with 2 members each" do
    ...
    assert TeamGenerator.allocate_members(get_teams(4), all_members) == expected_result
  end
